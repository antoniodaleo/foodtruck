...
public class FrmTipoPagamento extends JFrame {
	...
	public FrmTipoPagamento() {
		...
		btnExcluir.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int r = JOptionPane.showConfirmDialog(null, "Deseja excluir esse Tipo de Pagamento", "EXCLUIR TIPO DE PAGAMENTO", JOptionPane.YES_OPTION); 
				if (r== JOptionPane.YES_OPTION) {
					t.setId(Integer.parseInt(txtId.getText()));
					t.excluir();
					JOptionPane.showMessageDialog(null, "Tipo pagamento eliminado com sucesso!");
					dispose();
					FrmTipoPagamento p = new FrmTipoPagamento(); 
					p.setVisible(true);
					limpar();
					botoes(0);
					campos(0);
		...
		JLabel lblCadastroDoSetor = new JLabel("CADASTRO DO TIPO DE PAGAMENTO");
		...
		pnlCabecalho.add(lblCadastroDoSetor);
		{
			...
			pnlDados.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Tipo de pagamento", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (validarCampos()== true) {
					...
					if(edicao == false){
						...
						t.setId(Integer.parseInt(txtId.getText()));
						t.editar();
						JOptionPane.showMessageDialog(null, "Tipo de pagamento atualizado com sucesso!");
						dispose();
						FrmTipoPagamento ls = new FrmTipoPagamento(); 
						ls.setVisible(true);
						edicao = false;
						limpar();
						botoes(0);
